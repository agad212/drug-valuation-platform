...
const OPTION_COLORS = ["#10b981", "#3b82f6", "#f59e0b", "#ec4899"];
...
export default function DecisionAnalysis({ valuation, out, ptrsResult, layer2Result }: Props) {
  ...
  const base = useMemo(
    () => buildBaseContext(valuation, out, ptrsResult, layer2Result),
    [valuation, out, ptrsResult, layer2Result]
  );
  ...
  // Compute results from current option definitions
  const results = useMemo(() => {
    if (!base) return [];
    return computeAllOptions(base, options);
  }, [base, options]);
